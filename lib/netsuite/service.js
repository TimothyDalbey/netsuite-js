...
Service.prototype.upsert = function(Record) {
  assertConnection(this);
  this.config.setAuthHeader();
  var soapObj = Serializer.serialize(Record);
  var upsert = denodeify(this.config.client.upsert);
  return upsert(soapObj);
};

Service.prototype.initialize = function(Record) {
  assertConnection(this);
  this.config.setAuthHeader();
  var soapObj = Serializer.serialize(Record);
  var initialize = denodeify(this.config.client.initialize);
  return initialize(soapObj);
}

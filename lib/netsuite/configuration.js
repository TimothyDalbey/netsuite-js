...
var Configuration = module.exports = function Configuration(credentials, options) {
  ...
  // Default
  this._webservicesDomain = 'https://webservices.na2.netsuite.com/';
};


    // def self.generateTimestamp()
    //     Time.now.to_i.to_s
    // end

    // def self.generateNonce()
    //     alphanumeric = [*('0'..'9'), *('a'..'z')]
    //     (0...20).map { alphanumeric[rand(alphanumeric.length)] }.join
    // end
    ...
    function generateSignature(nonce, timestamp, credentials) {
      var signature_data = [
          credentials.account,
      ...
      var signature_key = `${credentials.consumer_secret}&${credentials.secret}`
      // debugger;

      // console.log(CryptoJS.HmacSHA256(signature_data, signature_key))
      // console.log(Base64.encode(CryptoJS.HmacSHA256(signature_data, signature_key)))
      return CryptoJS.HmacSHA256(signature_data, signature_key).toString(CryptoJS.enc.Base64)
      // return Base64.encode(CryptoJS.HmacSHA256(signature_data, signature_key).toString(CryptoJS.enc.Base64))

      // Base64.encode64(OpenSSL::HMAC.digest(OpenSSL::Digest.new('sha256'), signature_key, signature_data))
    }

    // def self.auth_header(user, password, search)
    //     if search
    //         preferences = {
    //                         'platformMsgs:bodyFieldsOnly' => false,
    //                         'platformMsgs:pageSize' => search,
    ...
    //     nonce = generateNonce()
    //     signature = generateSignature(nonce, timestamp, user, password)

    //     {
    //       'platformMsgs:tokenPassport' => {
    //         'platformCore:account'     => self::ACCOUNT_ID,
    //         'platformCore:consumerKey' => self::CONSUMER_KEY,
    //         'platformCore:token' => user,
    ...
    //       },
    //       'platformMsgs:applicationInfo' => preferences
    //     }
    // end
...
Configuration.createAuthHeader = function(credentials) {
  ...
  var date = new Date();
  var timestamp = parseInt( date.getTime() / 1000).toString();
  var nonce = randomString(20);
  var signature = generateSignature(nonce, timestamp, credentials)

  var soapObj = {
    'platformMsgs:tokenPassport': {
      'platformCore:account': credentials.account,
      'platformCore:consumerKey': credentials.consumer_key,
      'platformCore:token': credentials.token,
      'platformCore:nonce': nonce,
      'platformCore:timestamp': timestamp,
...
Configuration.prototype.setAuthHeader = function() {
  var _this = this;
  this.removeSoapHeader('platformMsgs:tokenPassport');
  this.removeSoapHeader('platformMsgs:applicationInfo');
